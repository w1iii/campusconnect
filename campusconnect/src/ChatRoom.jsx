...
function ChatRoom({ username }) {
  ...
  useEffect(() => {
    ...
    socket.on("reconnect", () => {
      console.log("♻️ Reconnected, rejoining queue...");
      joinQueue();
    });

    socket.on("matched", (partnerData) => {
      console.log("🎉 Matched with:", partnerData);
      setPartner({
    ...
    socket.on("receiveMessage", (data) => {
      setMessages((prev) => [
        ...prev,
        { from: "partner", text: data.text, username: data.username },
      ]);
    });
